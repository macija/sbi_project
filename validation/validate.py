...
def get_predicted_bs(prot_format, prot_path, o_path):
    """
    Return the predicted binding siteS for a given protein
    """
    model=PUResNet()
    model.load_weights('/home/marc/Documents/SBI_PYT_proj/BSPredictor/train_test_families_1rep_best_weights.h5')
    ...
    predicted_bs_filepaths = []
    for pocket in glob.glob(o_path+'/pocket*.mol2'):
        predicted_bs_filepaths.append(pocket)
    
    #os.path.join(o_path,"pocket0.mol2")
    
    return predicted_bs_filepaths
...
def validate(val_subset, output_folder):
    ...
    for root, dirs, _ in os.walk(val_subset, topdown=False):
        for dir in dirs: 
            ...
            # If cavities has been predicted

            predicted_bs_files = get_predicted_bs("mol2", protein_file, os.path.join(output_folder, dir))
            
            # Check if any cavity has been predicted
            dcc_current = []
            dvo_current = []
            
            for bs_file in predicted_bs_files: 
                ...
                # Get the grids
                actual_bs_coords, pred_bs_coords = get_bs_coords("mol2", actual_bs_file, bs_file)
            ...
            if len(dcc_current) > 0:
                dccs[dir] = min(dcc_current)
            if len(dvo_current) > 0:
                dvos[dir] = min(dvo_current)
    return dccs, dvos
...
if __name__=='__main__':
    #validate("", "output")
    dccs, dvos = validate('../validation_data/scPDB', '../validation_data/output_our')
